...
function getTime(date) {
  ...
  const hh = String(date2.getHours()).padStart(2, '0');
  const mm = String(date2.getMinutes()).padStart(2, '0');
  const ss = String(date2.getSeconds()).padStart(2, '0');
...
function getNextFriday(date) {
  const date2 = new Date(date);
  const dayNum = date2.getDay();
  const dayUntilFriday = (5 - dayNum + 7) % 7 || 7;

  date2.setDate(date2.getDate() + dayUntilFriday);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.abs((end - start) / msInDay) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  date2.setUTCDate(date2.getUTCDate() + (4 - dayNum));
  const firstJan = new Date(Date.UTC(date2.getUTCFullYear(), 0, 1));
  const weekNum = Math.ceil(((date2 - firstJan) / 86400000 + 1) / 7);
